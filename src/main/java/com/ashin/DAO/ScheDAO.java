...
public class ScheDAO {
    public ArrayList<Schedule> showClassSchedule(int idClass, int semester, int weekday) {
    ...
    public ArrayList<Schedule> showTeacherSchedule(int idTeacher, int semester, int weekday){
        Connection connect = null;
        PreparedStatement ps = null;
        ObjectPool pool = MyPool.getInstance();
        try {
            connect = (Connection) pool.borrowObject();
            ArrayList<Schedule> list = new ArrayList<Schedule>();
            ps = connect.prepareStatement("SELECT * FROM V_TKB WHERE V_TKB.MA_GV=? and V_TKB.HOC_KY=? and V_TKB.THU=?");
...
//        ArrayList<Schedule> list = new ArrayList<Schedule>();
//        list=scheduleDAO.showClassSchedule(1,1);
//        for (Schedule s:list) {
